...
from evaluator.metric import Metric
...
def compute_exact(a_gold, a_pred):
    print(f"# gold: {a_gold}, pred: {a_pred}, exact: {int(normalize_answer(a_gold) == normalize_answer(a_pred))}")
...
def compute_f1(a_gold, a_pred):
    ...
    print(f"# gold_toks: {gold_toks}, pred_toks: {pred_toks}, {precision=}, {recall=}, f1: {f1}")
    return f1
...
class SquadAnswerEmF1Metric(Metric):
    ...
    def __call__(
        ...
        print(f"predicted_answer: {predicted_answer}, ground_truth_answers: {ground_truth_answers}")
