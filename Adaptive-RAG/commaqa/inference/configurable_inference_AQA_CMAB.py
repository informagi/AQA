...
def build_decomposer_and_models(config_map):
    ...
    for key, value in config_map["models"].items():
        ...
        print(f"\n\n\n")
        print(f"Adding model: {key} of type: {class_name}")
        print(f"What is model.query: {model.query}")
...
def process_example(example, args, decomposer, model_map, override_answer_by):
    dataset = example.get('source_dataset', 'default')

    print(f"Processing example for dataset: {dataset}")
    print(f"example: {example}")
    config_map = adjust_config_for_dataset(dataset, args.config)
    decomposer, model_map = build_decomposer_and_models(config_map)
    
    return decomposer.return_qid_prediction(
        example, override_answer_by=override_answer_by, debug=args.debug, silent=args.silent
    )

def adjust_config_for_dataset(dataset, base_config_path):
    # with open(base_config_path, 'r') as file:
    #     config_content = file.read()
    
    ext_vars = get_environment_variables()
    config_map = json.loads(_jsonnet.evaluate_file(base_config_path, ext_vars=ext_vars))
    rc_context_type = config_map.get("rc_context_type", "default_value")
    print(f"rc_context_type: {rc_context_type}")
    print(f"config_map: {config_map}")
    # Adjust the config_map based on the dataset
    if dataset == "hotpotqa":
        if "generate_titles" in config_map["models"]:
            config_map["models"]["generate_titles"]["source_corpus_name"] = "hotpotqa"
        # Change the dataset part of the prompt file path to hotpotqa in the config_map for the following models the dataset name comes after prompt
        if "step_by_step_cot_reasoning_gen" in config_map["models"]:
            # splitted_step_by_step_cot_reasoning_gen_prompt_file = config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"].split("/")
            # splitted_step_by_step_cot_reasoning_gen_prompt_file[1] = "hotpotqa"
            # config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"] = "/".join(splitted_step_by_step_cot_reasoning_gen_prompt_file)
            config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"] = "prompts/hotpotqa/gold_with_3_distractors_context_direct_qa_flan_t5.txt"

        # if  config_map["models"]["answer_main_question"]:
        if "answer_main_question" in config_map["models"]:
            # splitted_answer_main_question_prompt_file = config_map["models"]["answer_main_question"]["prompt_file"].split("/")
            # splitted_answer_main_question_prompt_file[1] = "hotpotqa"
            # config_map["models"]["answer_main_question"]["prompt_file"] = "/".join(splitted_answer_main_question_prompt_file)
            config_map["models"]["answer_main_question"]["prompt_file"] = "prompts/hotpotqa/gold_with_3_distractors_context_cot_qa_flan_t5.txt"

    elif dataset == "musique":
        if "generate_titles" in config_map["models"]:
            config_map["models"]["generate_titles"]["source_corpus_name"] = "musique"
        if "step_by_step_cot_reasoning_gen" in config_map["models"]:
            # splitted_step_by_step_cot_reasoning_gen_prompt_file = config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"].split("/")
            # splitted_step_by_step_cot_reasoning_gen_prompt_file[1] = "musique"
            # config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"] = "/".join(splitted_step_by_step_cot_reasoning_gen_prompt_file)
            config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"] = "prompts/hotpotqa/gold_with_3_distractors_context_direct_qa_flan_t5.txt"
        ...
        if "answer_main_question" in config_map["models"]:
            # splitted_answer_main_question_prompt_file = config_map["models"]["answer_main_question"]["prompt_file"].split("/")
            # splitted_answer_main_question_prompt_file[1] = "musique"
            # config_map["models"]["answer_main_question"]["prompt_file"] = "/".join(splitted_answer_main_question_prompt_file)
            config_map["models"]["answer_main_question"]["prompt_file"] = "prompts/hotpotqa/gold_with_3_distractors_context_cot_qa_flan_t5.txt"
        ...
        if "generate_titles" in config_map["models"]:
            ...
            config_map["models"]["generate_titles"]["source_corpus_name"] = "2wikimultihopqa"

        if "step_by_step_cot_reasoning_gen" in config_map["models"]:
            # splitted_step_by_step_cot_reasoning_gen_prompt_file = config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"].split("/")
            # splitted_step_by_step_cot_reasoning_gen_prompt_file[1] = "2wikimultihopqa"
            # config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"] = "/".join(splitted_step_by_step_cot_reasoning_gen_prompt_file)
            config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"] = "prompts/hotpotqa/gold_with_3_distractors_context_direct_qa_flan_t5.txt"
        # if  config_map["models"]["answer_main_question"]:
        if "answer_main_question" in config_map["models"]:
            # splitted_answer_main_question_prompt_file = config_map["models"]["answer_main_question"]["prompt_file"].split("/")
            # splitted_answer_main_question_prompt_file[1] = "2wikimultihopqa"
            # config_map["models"]["answer_main_question"]["prompt_file"] = "/".join(splitted_answer_main_question_prompt_file)
            config_map["models"]["answer_main_question"]["prompt_file"] = "prompts/hotpotqa/gold_with_3_distractors_context_cot_qa_flan_t5.txt"
    elif dataset in ["squad", "nq", "trivia"]:
        if "generate_titles" in config_map["models"]:
            config_map["models"]["generate_titles"]["source_corpus_name"] = "wiki"
        if "step_by_step_cot_reasoning_gen" in config_map["models"]:
            # splitted_step_by_step_cot_reasoning_gen_prompt_file = config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"].split("/")
            # splitted_step_by_step_cot_reasoning_gen_prompt_file[1] = dataset
            # config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"] = "/".join(splitted_step_by_step_cot_reasoning_gen_prompt_file)
            config_map["models"]["step_by_step_cot_reasoning_gen"]["prompt_file"] = "prompts/hotpotqa/gold_with_3_distractors_context_direct_qa_flan_t5.txt"
        ...
        if "answer_main_question" in config_map["models"]:
            # splitted_answer_main_question_prompt_file = config_map["models"]["answer_main_question"]["prompt_file"].split("/")
            # splitted_answer_main_question_prompt_file[1] = dataset
            # config_map["models"]["answer_main_question"]["prompt_file"] = "/".join(splitted_answer_main_question_prompt_file)
            config_map["models"]["answer_main_question"]["prompt_file"] = "prompts/hotpotqa/gold_with_3_distractors_context_cot_qa_flan_t5.txt"
...
def inference_mode(args, reader, decomposer, model_map, override_answer_by=None):
    ...
    if args.threads > 1:
        ...
        for idx, example in enumerate(iterator):
            # if idx >= 12:  # Stop after 3 examples
                # break
            print(f"\nIndex: {idx}")
            print(f"Processing question: {example.get('query', 'No query found')} with QID: {example.get('qid', 'No QID found')}")
            ...
            #     )
            # )
            # qid_answer_chains.append(process_example(example, args, decomposer, model_map, override_answer_by))
            final_state = process_example(example, args, decomposer, model_map, override_answer_by)
            # Extract conf_info and run_time_in_seconds from the state
            conf_info = final_state.data.get('conf_info', 'No conf_info')
            run_time = final_state.data.get('run_time_in_seconds', 'No run time')
            qid_answer_chains.append((example.get('qid', 'No QID found'), final_state.data['answer'], final_state.data['chain'], conf_info, run_time))
    ...
    with open(args.output[:ext_index] + "_confidence_complete.txt", "w") as output_fp:
        json.dump({x[0]: {"answer": x[1], "conf_info": x[3], "run_time_in_seconds": x[4]} for x in qid_answer_chains}, output_fp, indent=4)
    

    with open(args.output[:ext_index] + "_confidence_complete_per_Chain.txt", "w") as output_fp:
        for chain in qid_answer_chains:
            output_fp.write(f"QID: {chain[0]}, Chain: {chain[2]}, Conf: {chain[3]}, Time: {chain[4]}\n")
